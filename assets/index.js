document.addEventListener('DOMContentLoaded', function(){
  ...
  function Update() {
    if (URLSections.length > 0) {
      FullURL = `${window.location.origin}/view.html?viewType=${TypeInput.options[TypeInput.selectedIndex].value}&tools=${URLSections.join(',')}`
    ...
    if (URLSections.length === Object.keys(Tools).length) {
      FullURL = `${window.location.origin}/view.html?viewType=${TypeInput.options[TypeInput.selectedIndex].value}&tools=*`
      URLInput.value = FullURL
    }
  }
